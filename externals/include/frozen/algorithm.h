...
template <std::size_t size> class knuth_morris_pratt_searcher {
    ...
    static constexpr bits::carray<std::ptrdiff_t, size> build_kmp_cache(char const (&needle)[size + 1]) {
        std::ptrdiff_t cnd = 0;
  ...
  public:
    ...
    constexpr std::pair<ForwardIterator, ForwardIterator> operator()(ForwardIterator first,
...
template <std::size_t size> class boyer_moore_searcher {
    ...
    constexpr std::size_t suffix_length(char const (&needle)[size + 1], std::size_t pos) {
...
template <std::size_t N> constexpr boyer_moore_searcher<N - 1> make_boyer_moore_searcher(char const (&needle)[N]) {
    return {needle};
}
