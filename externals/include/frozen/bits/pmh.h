...
template <size_t M> struct pmh_buckets {
    ...
    template <std::size_t... Is> carray<bucket_ref, M> constexpr make_bucket_refs(std::index_sequence<Is...>) const {
...
template <class T, size_t N> constexpr bool all_different_from(cvector<T, N> &data, T &a) {
...
template <std::size_t M, class Hasher> struct pmh_tables {
    ...
    template <typename KeyType> constexpr std::size_t lookup(const KeyType &key) const {
...
pmh_tables<M, Hash> constexpr make_pmh_tables(const carray<Item, N> &items, Hash const &hash, Key const &key, PRG prg) {
