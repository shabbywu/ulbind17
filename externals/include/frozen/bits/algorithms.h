...
constexpr std::size_t bit_weight(std::size_t n) {
    return (n <= 8 * sizeof(unsigned int)) + (n <= 8 * sizeof(unsigned long)) + (n <= 8 * sizeof(unsigned long long)) +
           (n <= 128);
}
...
template <std::size_t N> unsigned long long select_uint_least(std::integral_constant<std::size_t, N>) {
...
template <class T, class U> constexpr void cswap(std::pair<T, U> &a, std::pair<T, U> &b) {
...
template <class... Tys> constexpr void cswap(std::tuple<Tys...> &a, std::tuple<Tys...> &b) {
...
template <class T, class Compare> struct LowerBound {
    ...
    inline constexpr ForwardIt doit_fast(ForwardIt first, std::integral_constant<std::size_t, N>) {
    ...
    template <class ForwardIt, std::size_t N>
    inline constexpr ForwardIt doitfirst(ForwardIt first, std::integral_constant<std::size_t, N>,
    ...
    inline constexpr ForwardIt doitfirst(ForwardIt first, std::integral_constant<std::size_t, N>,
        ...
        if (compare_(*it, value_)) {
            ...
            return doitfirst(it + 1, std::integral_constant<std::size_t, next>{},
                             std::integral_constant<bool, next_highest_power_of_two(next) - 1 == next>{});
        } else
            return doit_fast(first, std::integral_constant<std::size_t, next_start>{});
    }

    template <class ForwardIt>
    inline constexpr ForwardIt doitfirst(ForwardIt first, std::integral_constant<std::size_t, 1>,
                                         std::integral_constant<bool, false>) {
