...
template <class Comparator> class CompareKey {
  ...
  public:
    constexpr CompareKey(Comparator const &comparator) : comparator_(comparator) {
    }

    template <class Key1, class Key2, class Value>
    constexpr int operator()(std::pair<Key1, Value> const &self, std::pair<Key2, Value> const &other) const {
        return comparator_(std::get<0>(self), std::get<0>(other));
    }

    template <class Key1, class Key2, class Value>
    constexpr int operator()(Key1 const &self_key, std::pair<Key2, Value> const &other) const {
        return comparator_(self_key, std::get<0>(other));
    }

    template <class Key1, class Key2, class Value>
    constexpr int operator()(std::pair<Key1, Value> const &self, Key2 const &other_key) const {
        return comparator_(std::get<0>(self), other_key);
    }

    template <class Key1, class Key2> constexpr int operator()(Key1 const &self_key, Key2 const &other_key) const {
...
template <class Key, class Value, std::size_t N, class Compare = std::less<Key>> class map {
  ...
  public:
    ...
    constexpr map(std::initializer_list<value_type> items) : map{items, Compare{}} {
    }
    ...
    template <class KeyType> constexpr std::size_t count(KeyType const &key) const {
        return bits::binary_search<N>(items_.begin(), key, less_than_);
    }

    template <class KeyType> constexpr const_iterator find(KeyType const &key) const {
    ...
    template <class KeyType> constexpr std::pair<const_iterator, const_iterator> equal_range(KeyType const &key) const {
        return equal_range_impl(*this, key);
    }
    template <class KeyType> constexpr std::pair<iterator, iterator> equal_range(KeyType const &key) {
        return equal_range_impl(*this, key);
    }

    template <class KeyType> constexpr const_iterator lower_bound(KeyType const &key) const {
    ...
    template <class KeyType> constexpr const_iterator upper_bound(KeyType const &key) const {
  ...
  private:
    template <class This, class KeyType> static inline constexpr auto &at_impl(This &&self, KeyType const &key) {
    ...
    template <class This, class KeyType> static inline constexpr auto find_impl(This &&self, KeyType const &key) {
        auto where = self.lower_bound(key);
...
template <class Key, class Value, class Compare> class map<Key, Value, 0, Compare> {
  ...
  public:
    ...
    constexpr map(std::initializer_list<value_type>, Compare const &compare) : less_than_{compare} {
    }
    constexpr map(std::initializer_list<value_type> items) : map{items, Compare{}} {
    }
    ...
    template <class KeyType> constexpr const_iterator find(KeyType const &) const {
        return end();
    }
    ...
    template <class KeyType> constexpr std::pair<const_iterator, const_iterator> equal_range(KeyType const &) const {
        return {end(), end()};
    }
    template <class KeyType> constexpr std::pair<iterator, iterator> equal_range(KeyType const &) {
        return {end(), end()};
    }

    template <class KeyType> constexpr const_iterator lower_bound(KeyType const &) const {
        return end();
    }
    ...
    template <class KeyType> constexpr const_iterator upper_bound(KeyType const &) const {
        return end();
    }
    ...
    constexpr key_compare const &value_comp() const {
        return less_than_;
    }
};
...
template <typename T, typename U, std::size_t N> constexpr auto make_map(std::pair<T, U> const (&items)[N]) {
    return map<T, U, N>{items};
}

template <typename T, typename U, std::size_t N> constexpr auto make_map(std::array<std::pair<T, U>, N> const &items) {
    return map<T, U, N>{items};
}
