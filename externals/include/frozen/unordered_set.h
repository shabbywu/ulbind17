...
class unordered_set {
  ...
  public:
    ...
    constexpr unordered_set(container_type keys, Hash const &hash, KeyEqual const &equal)
        : equal_{equal}, keys_{keys},
          tables_{bits::make_pmh_tables<storage_size>(keys_, hash, bits::Get{}, default_prg_t{})} {
    }
...
template <typename T, std::size_t N> constexpr auto make_unordered_set(T const (&keys)[N]) {
...
template <typename T, std::size_t N> constexpr auto make_unordered_set(std::array<T, N> const &keys) {
    return unordered_set<T, N>{keys};
}
...
template <class T, class... Args> unordered_set(T, Args...) -> unordered_set<T, sizeof...(Args) + 1>;
