...
///
template <typename T> class LockedPixels;
...
class UExport Bitmap : public RefCounted {
  public:
    ...
    /// @return  A ref-pointer to a new Bitmap instance.
    ///
    static RefPtr<Bitmap> Create(uint32_t width, uint32_t height, BitmapFormat format, uint32_t row_bytes,
                                 const void *pixels, size_t size, bool should_copy = true);
...
template <typename T> class LockedPixels {
  public:
    ...
    explicit LockedPixels(T &lockable) : lockable_(lockable), data_(nullptr), size_(0) {
        lock();
    }
    ...
    LockedPixels(LockedPixels &&other) : lockable_(other.lockable_), data_(other.data_), size_(other.size_) {
