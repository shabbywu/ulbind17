...
struct UExport vec4 {
    ...
    inline friend vec4 operator+(vec4 lhs, const vec4 &rhs) {
        lhs += rhs;
        return lhs;
    }
    ...
    inline friend vec4 operator+(vec4 lhs, float rhs) {
        lhs += rhs;
        return lhs;
    }
...
struct UExport Rect {
    ...
    inline bool Intersects(const Rect &rhs) const {
        return !(rhs.left > right || rhs.right < left || rhs.top > bottom || rhs.bottom < top);
    }

    inline Rect Intersect(const Rect &other) const {
        return {(left < other.left) ? other.left : left, (top < other.top) ? other.top : top,
                (other.right < right) ? other.right : right, (other.bottom < bottom) ? other.bottom : bottom};
    }
...
struct UExport IntRect {
    ...
    inline Point origin() const {
        return Point{(float)left, (float)top};
    }
